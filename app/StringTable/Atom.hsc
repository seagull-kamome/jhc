...
import qualified Data.ByteString.UTF8 as BS(fromString,toString) -- utf8-string package
...
instance Semigroup Atom where x <> y = unsafePerformIO $ atomAppend x y
instance Monoid Atom where mempty = Atom (#const ATOM_EMPTY)
instance IsString Atom where fromString = toAtom
instance Show Atom where showsPrec _ atom = (fromAtom atom ++)
instance Read Atom where readsPrec _ s = [ (toAtom s,"") ]
