...
data Flag =
    ...
    | MonoLocalBinds    -- ^ make local bindings monomorphic
    ...
    | Sugar             -- ^ disable\/enable all desugarings, only unboxed literals allowed.
...
instance Show Flag where
    ...
    show Lint = "lint"
    show M4 = "m4"
    show MonoLocalBinds = "mono-local-binds"
...
one "default" = Right $ foldr (.) id [ f | Right f <- [ one "inline-pragmas",one "rules",one "wrapper",one "type-analysis",one "global-optimize",one "full-int"]]
...
one "mono-local-binds" = Right $ Set.insert MonoLocalBinds
one "no-mono-local-binds" = Right $ Set.delete MonoLocalBinds
