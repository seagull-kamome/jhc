...
setTempDir (FP.normalise -> fp) = do
    ...
    when (isJust tempDirPath) $
      fail $ printf "Cannot set temp directory to '%s'; it is already set to '%s'." fp (fromJust tempDirPath)
...
cleanUp = do
    td <- readIORef tdRef
    tempDirAtExit td
    when (not (tempDirClean td) || isNothing (tempDirPath td)) $ do
      dir <- getTempDir
