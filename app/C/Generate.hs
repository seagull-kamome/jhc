...
generateC fs ss = ans where
    G ga = do
        ...
        let shead = vcat [ text "struct" <+> text (show $ structureName s) <> (if structureAligned s then text " A_ALIGNED" else empty) <> text ";" | s <- ss ]
    ...
    declStructs ss = liftM vsep $ forM ss $ \ s@Structure { structureName = n, structureFields = ts } -> do
        ...
        ts' <- forM (tsort ts) $ \ (n,t) -> do
            t <- draw t
            return $ t <+> text (show n) <> semi
        return $ text "struct" <+> text (show n) <+> lbrace <$$> nest 4 (vcat $ (if structureNeedsDiscriminator s  then text "what_t what;" else empty):ts') <$$> rbrace <> semi
