...
newtype TyLevel = TyLevel Int deriving(Eq,Ord,Enum)
...
class HasTyLevel a where
    ...
    tyLevelOf x = fromMaybe (error "tyLevelOf: Does not have a TyLevel") $ getTyLevel x
...
termLevel, typeLevel, kindLevel :: TyLevel
termLevel = TyLevel 0
typeLevel = TyLevel 1
kindLevel = TyLevel 2
