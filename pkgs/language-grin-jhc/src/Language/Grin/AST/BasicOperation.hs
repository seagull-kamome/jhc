...
data BasicOperation'' typ
  ...
  | Apply [typ] -- apply a partial application to a value, returning the given type
  ...
  | Coerce typ -- coerce one type to another, danger zone. This is for reflection/rts and not for integral conversions.
...
type BasicOperation primtypes = BasicOperation'' (Typ primoperation)
